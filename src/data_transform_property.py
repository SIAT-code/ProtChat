...
def reg_pack(pro_tokens, go_tokens, pro_ids, goterms, labels, dataset_path):
...
def cls_pack(pro_tokens, go_tokens, pro_ids, goterms, labels, dataset_path):
    pro2id_path = os.path.join(dataset_path, "pro2id.json")
    with open(pro2id_path) as f:
        pro2id_dict = json.load(f)
    go2id_path = os.path.join(dataset_path, "go2id.json")
...
def multilabel_pack(pro_tokens, go_tokens, pro_ids, goterms, labels, dataset_path):
    pro2id_path = os.path.join(dataset_path, "pro2id.json")
    with open(pro2id_path) as f:
        pro2id_dict = json.load(f)
    go2id_path = os.path.join(dataset_path, "go2id.json")
...
def transform_data(dataset, max_pro_seq_len, dataset_path, task_type):
    ...
    if task_type in ['stability', 'fluorescence']:
        data_pack_df = reg_pack(seq_tokens, go_tokens, pro_sequences, goterms, labels, dataset_path)
        dataset_pack = RegDataset(data_pack_df)
    elif ('remote' in task_type or 'homology' in task_type) or ('antigen' in task_type or 'binding' in task_type):
        data_pack_df = cls_pack(seq_tokens, go_tokens, pro_sequences, goterms, labels, dataset_path)
        dataset_pack = ClsDataset(data_pack_df)
    elif 'secondary' in task_type or 'structure' in task_type:
        data_pack_df = multilabel_pack(seq_tokens, go_tokens, pro_sequences, goterms, labels, dataset_path)
        dataset_pack = MultiLabelDataset(data_pack_df)
    else:
...
def data_read(dir_input, file_path, dataset_type, task_type, max_pro_seq_len, seed, shuffle=True):
    df_tokens = pd.read_csv(file_path, sep=',')
    df_tokens = df_tokens[df_tokens['set'] == dataset_type]
    df_tokens.reset_index(drop=True, inplace=True)
    if len(df_tokens) == 0:
        return []
    
    seq_tokens, go_tokens, labels = df_tokens['seq'].values.tolist(), df_tokens['go'].values.tolist(), df_tokens['label'].values.tolist()
    seq_tokens_tmp = []
    for seq_token in seq_tokens:
        seq_tokens_tmp.append(seq_token[: max_pro_seq_len])
    seq_tokens = seq_tokens_tmp
    if 'secondary' in task_type or 'structure' in task_type:
        labels_tmp = []
        for label in labels:
    ...
    pro_ids = load_tensor_for_pro_ids(os.path.join(dir_input, 'seqs_id_%s.txt' % dataset_type), torch.FloatTensor)

    goterms = load_tensor(os.path.join(dir_input, 'goterms_%s' % dataset_type), torch.FloatTensor)

    cath = CATHDataset(os.path.join(dir_input, 'structure_%s.jsonl' % dataset_type))
