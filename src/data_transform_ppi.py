...
def data_read(dir_input, file_path, dataset_type, max_pro_seq_len, seed, shuffle=True):
    ...
    if 'fold' in dataset_type:
        fold_i, dataset_type = dataset_type.split('-')
        fold_i = fold_i.replace('fold', '')
        indices = df_tokens.index[df_tokens[f'set{fold_i}'] == dataset_type].tolist()
        df_tokens = df_tokens[df_tokens[f'set{fold_i}'] == dataset_type]
    else:
        indices = df_tokens.index[df_tokens['set'] == dataset_type].tolist()
        df_tokens = df_tokens[df_tokens['set'] == dataset_type]
    df_tokens.reset_index(drop=True, inplace=True)
    if len(df_tokens) == 0:
        return []
    
    seq_tokens_1, go_tokens_1, pro_ids_1, seq_tokens_2, go_tokens_2, pro_ids_2, labels = df_tokens['seq1'].fillna('').values.tolist(), df_tokens['go1'].fillna('').values.tolist(), df_tokens['pro_id1'].fillna('').values.tolist(), df_tokens['seq2'].fillna('').values.tolist(), df_tokens['go2'].fillna('').values.tolist(), df_tokens['pro_id2'].fillna('').values.tolist(), df_tokens['label'].fillna('').values.tolist()
    ...
    goterms_1 = load_tensor(os.path.join(dir_input, 'goterms_all_1'), torch.FloatTensor)
    goterms_2 = load_tensor(os.path.join(dir_input, 'goterms_all_2'), torch.FloatTensor)
    goterms_1  =[goterms_1[i] for i in indices]
    goterms_2  =[goterms_2[i] for i in indices]
    
    cath_1 = CATHDataset(os.path.join(dir_input, 'structure_all_1.jsonl'))
    cath_2 = CATHDataset(os.path.join(dir_input, 'structure_all_2.jsonl'))
    cath_1.data = [cath_1.data[i] for i in indices]
    cath_2.data = [cath_2.data[i] for i in indices]
