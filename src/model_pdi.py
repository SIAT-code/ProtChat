...
class ProLigandCrossFusion(nn.Module):
    ...
    def forward(self, pro_feat, compound_feat, pro_mask=None, compound_mask=None):
        ...
        for layer in self.pro_layers:
            # pro_feat_new = layer(pro_feat, com_feat_for_pro, pro_mask, compound_mask.permute(0, 1, 3, 2))
        ...
        for layer in self.compound_layers:
            # compound_feat_new = layer(compound_feat, pro_feat_for_com, compound_mask, pro_mask.permute(0, 1, 3, 2))
...
class ClassificationLayer(nn.Module):
    def __init__(self, pro_hid_dim, label_num):
        super().__init__()
        self.fc1 = nn.Linear(pro_hid_dim, int(pro_hid_dim/2))
        self.fc2 = nn.Linear(int(pro_hid_dim/2), label_num)
...
class Predictor(nn.Module):
    def __init__(self, seq_encoder, struc_encoder, seq_struc_fusion, goterm_encoder, pro_go_cross_fusion, compound_encoder, pro_ligand_cross_fusion, regression_layer, add_structure, add_goterm, task_type):
        super().__init__()
        ...
        self.classification_layer = regression_layer
    ...
    def forward(self, seq_feat, struc_feat, proteins_num, goterms, goterms_num, compounds_num, atoms, bonds, atom_degrees, bond_degrees, atom_masks):
        ...
        if self.task_type in ['pdbbind']:
            ...
            label = self.classification_layer(pro_emb, mol_emb, proteins_mask)
        elif self.task_type in ['kinase']:
...
def create_model(max_pro_seq_len, add_structure, add_goterm, task_type):
    ...
    # Protein and Structure Fusion Parameters
    protein_dim = 512
    seq_struc_n_heads = 8
    ...
    # Protein and Goterm Fusion Parameters
    fusion_pro_n_layers = 3
    fusion_go_n_layers = 3

    # Compound Encoder Parameters
    radius = 3
    T = 1
    input_atom_dim = 39
    input_bond_dim = 10
    ...
    # # Protein and Compound Fusion Parameters
    ...
    if task_type in ['pdbbind']:
        ...
        regression_layer = ClassificationLayer(seq_hid_dim, 2)
    else:
        raise ValueError("Unkown task type.")
    
    model = Predictor(seq_encoder, struc_encoder, seq_struc_fusion, goterm_encoder, pro_go_cross_fusion, compound_encoder, pro_ligand_cross_fusion, regression_layer, add_structure, add_goterm, task_type)
