...
def reg_pack(pro_tokens, go_tokens, compound_tokens, pro_ids, goterms, labels, dataset_path, dataset_type):
...
def cls_pack(pro_tokens, go_tokens, compound_tokens, pro_ids, goterms, labels, dataset_path, dataset_type):
    pro2id_path = os.path.join(dataset_path, "pro2id.json")
    with open(pro2id_path) as f:
        pro2id_dict = json.load(f)
    go2id_path = os.path.join(dataset_path, "go2id.json")
...
def transform_data(dataset, max_pro_seq_len, dataset_path, dataset_type, task_type):
    ...
    for data in dataset:
        ...
        labels.append(label)
    if task_type in ['pdbbind']:
        data_pack_df = reg_pack(seq_tokens, go_tokens, compound_tokens, pro_sequences, goterms, labels, dataset_path, dataset_type)
        dataset_pack = RegDataset(data_pack_df)
    elif task_type in ['kinase']:
        data_pack_df = cls_pack(seq_tokens, go_tokens, compound_tokens, pro_sequences, goterms, labels, dataset_path, dataset_type)
...
def data_read(dir_input, file_path, dataset_type, max_pro_seq_len, seed, shuffle=True):
    ...
    seq_tokens, go_tokens, compound_tokens, labels = df_tokens['seq'].fillna('').values.tolist(), df_tokens['go'].fillna('').values.tolist(), df_tokens['smile'].fillna('').values.tolist(), df_tokens['label'].fillna('').values.tolist()

    _, problem_indexs = search_problem_smiles(compound_tokens, os.path.join(dir_input, 'smiles_feature_dict_%s.pkl' % dataset_type))
