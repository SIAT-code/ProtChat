...
class Trainer(object):
    def __init__(self, model, lr, weight_decay, batch_size, gradient_accumulation, task_type):
        self.model = model
        ...
        if task_type in ['stability', 'fluorescence']:
            self.loss_fn = nn.MSELoss()
                ...
                or ('antigen' in task_type or 'binding' in task_type):
            self.loss_fn =nn.CrossEntropyLoss()
        else:
            raise ValueError("Unkown task type.")
    
    def train(self, dataset_tuple, device, dir_input, dataset_type):
        self.model.train()
        ...
        all_predict_labels, all_real_labels, all_pro_ids = [], [], []
        for step, batch in enumerate(zip(dataloader, dataloaderstruc)):
            ...
            if 'secondary' in self.task_type or 'structure' in self.task_type:
            ...
            for pro_id in pro_ids:
                seq_emb_path = os.path.join(os.path.join(dir_input, 'sequences_emb_%s' % dataset_type), pro_id+".npy")
            ...
            if 'secondary' in self.task_type or 'structure' in self.task_type:
                predict_labels_clone, labels_clone = predict_labels.clone(), labels.clone()
                predict_labels, labels = remove_no_loss_calculation(predict_labels, labels, cate_num=3)

            loss = self.loss_fn(predict_labels, labels)

            if 'secondary' in self.task_type or 'structure' in self.task_type:
                for i in range(len(predict_labels_clone)):
                    predict_label, label = predict_labels_clone[i].unsqueeze(0), labels_clone[i].unsqueeze(0)
                    predict_label, label = remove_no_loss_calculation(predict_label, label, cate_num=3)
                    predict_label = torch.argmax(predict_label, 1)
                    all_predict_labels.append(predict_label.tolist())
                    all_real_labels.append(label.tolist())
            else:
                if 'remote' in self.task_type or 'homology' in self.task_type:
                    predict_labels = torch.argmax(predict_labels, 1)
                elif ('antigen' in self.task_type or 'binding' in self.task_type):
        ...
        return loss_total/(step+1), all_predict_labels, all_real_labels, all_pro_ids


class Tester(object):
    def __init__(self, model, batch_size, task_type):
        self.model = model
        self.batch_size = batch_size
        self.task_type = task_type
        if task_type in ['stability', 'fluorescence']:
    ...
    def test(self, dataset_tuple, device, dir_input, dataset_type):
        self.model.eval()
        dataset, dataset_structure = dataset_tuple
        datasampler = SequentialSampler(dataset)
        dataloader = DataLoader(dataset, sampler=datasampler, batch_size=self.batch_size)
